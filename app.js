...
(function(){
  ...
  const LS_CALC='drc_calc_inputs',LS_TRACKER='drc_tracker_inputs',LS_WALLS='drc_walls_data',MAX_WALLS=20;
  ...
  function initPlanTool(){
    ...
    if(canvas){
      let pinchStartDist=0,pinchStartZoom=1,isTouchMoving=false;
      canvas.addEventListener('touchstart',function(e){
        if(e.touches.length===2){const dx=e.touches[0].clientX-e.touches[1].clientX,dy=e.touches[0].clientY-e.touches[1].clientY;pinchStartDist=Math.sqrt(dx*dx+dy*dy);pinchStartZoom=planState.zoom;e.preventDefault();}
        else if(e.touches.length===1){isTouchMoving=false;}
      },{passive:false});
      canvas.addEventListener('touchmove',function(e){
        if(e.touches.length===2){const dx=e.touches[0].clientX-e.touches[1].clientX,dy=e.touches[0].clientY-e.touches[1].clientY;const dist=Math.sqrt(dx*dx+dy*dy);if(pinchStartDist>0){applyZoom(pinchStartZoom*(dist/pinchStartDist));}e.preventDefault();}
        else if(e.touches.length===1){isTouchMoving=true;}
      },{passive:false});
      canvas.addEventListener('touchend',function(e){
        if(e.touches.length===0&&e.changedTouches.length===1&&!isTouchMoving&&planState.mode){const fake={clientX:e.changedTouches[0].clientX,clientY:e.changedTouches[0].clientY,preventDefault:()=>{}};onCanvasTap(fake);}
        pinchStartDist=0;
      },{passive:false});
