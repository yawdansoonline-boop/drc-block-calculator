...
(function(){
  ...
  function initPlanTool(){
    ...
    if(canvas){
      ...
      const LOUPE_R=60,LOUPE_Z=3;

      function updateCanvasTouch(){
        canvas.style.touchAction=planState.mode?'none':'pan-x pan-y pinch-zoom';
      }
      ...
      function showLoupe(cx,cy){
        ...
        loupe.style.top=Math.max(10,cy-LOUPE_R-140)+'px';
        const lCtx=loupeCanvas.getContext('2d');
        const srcS=LOUPE_R*2/LOUPE_Z;
        lCtx.clearRect(0,0,120,120);
        lCtx.drawImage(canvas,canX-srcS/2,canY-srcS/2,srcS,srcS,0,0,120,120);
      }
      function hideLoupe(){if(loupe)loupe.style.display='none';}

      // PINCH ZOOM on wrapper - always works
      wrap.addEventListener('touchstart',function(e){
        if(e.touches.length===2){
          const dx=e.touches[0].clientX-e.touches[1].clientX,dy=e.touches[0].clientY-e.touches[1].clientY;
          pinchStartDist=Math.sqrt(dx*dx+dy*dy);pinchStartZoom=planState.zoom;
          e.preventDefault();
        }
      },{passive:false});
      ...
      wrap.addEventListener('touchend',function(e){pinchStartDist=0;},{passive:false});

      // LOUPE + TAP on canvas - only in scale/measure mode
      canvas.addEventListener('touchstart',function(e){
        if(!planState.mode||e.touches.length!==1)return;
        e.preventDefault();showLoupe(e.touches[0].clientX,e.touches[0].clientY);
      },{passive:false});
      ...
      canvas.addEventListener('touchend',function(e){
        if(!planState.mode)return;
        e.preventDefault();hideLoupe();
    ...
    const btnS=document.getElementById('btnSetScale');if(btnS)btnS.addEventListener('click',()=>{planState.mode='scale';planState.points=[];document.getElementById('scaleInput').style.display='none';if(typeof updateCanvasTouch==='function')updateCanvasTouch();updateInstructions('<span class="step-badge">1</span> Tap <strong>first point</strong> on a known wall');});
    const btnM=document.getElementById('btnMeasure');if(btnM)btnM.addEventListener('click',()=>{if(!planState.scale){updateInstructions('<span class="step-badge">!</span> Set scale first!');return;}planState.mode='measure';planState.points=[];if(typeof updateCanvasTouch==='function')updateCanvasTouch();updateInstructions(`<span class="step-badge">3</span> Tap <strong>start</strong> of Wall ${planState.walls.length+1}`);});
    const btnAD=document.getElementById('btnAutoDetect');if(btnAD)btnAD.addEventListener('click',runAutoDetect);
    const btnC=document.getElementById('btnClear');if(btnC)btnC.addEventListener('click',()=>{resetPlanState();if(typeof updateCanvasTouch==='function')updateCanvasTouch();updateInstructions('<span class="step-badge">1</span> Tap <strong>SET SCALE</strong> or <strong>🔍 AUTO-DETECT</strong>');updateMeasuredWallsList();drawPlan();document.getElementById('autoDetectSection').style.display='none';const be=document.getElementById('btnEnhance');if(be){be.style.background='var(--white)';be.style.color='var(--mid)';}});
