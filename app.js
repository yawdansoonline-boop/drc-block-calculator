...
(function(){
  ...
  function getCanvasPoint(e){const canvas=document.getElementById('planCanvas');if(!canvas)return null;const rect=canvas.getBoundingClientRect();const scaleX=canvas.width/rect.width,scaleY=canvas.height/rect.height;const touch=e.touches?e.touches[0]:e;return{x:(touch.clientX-rect.left)*scaleX,y:(touch.clientY-rect.top)*scaleY};}

  function onCanvasTap(e){
    e.preventDefault();const pt=getCanvasPoint(e);if(!pt)return;
  ...
  function initPlanTool(){
    ...
    if(canvas){canvas.addEventListener('click',onCanvasTap);canvas.addEventListener('touchend',function(e){if(e.changedTouches&&e.changedTouches.length>0){const fake={clientX:e.changedTouches[0].clientX,clientY:e.changedTouches[0].clientY,preventDefault:()=>e.preventDefault()};onCanvasTap(fake);}},{passive:false});}
